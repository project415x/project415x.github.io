...
Sauron.prototype.blink = function(id){
  ...
  (function repeat(){
    ...
    x = wraith.attr("class");
    timer = util.getDistance(this.x, this.y, x, y);
    console.log("timer"+x);
...
Sauron.prototype.updateTargets = function(d, type) {
  ...
  for ( elem in list ) {
    ...
    if (util.isInRange(i[0], i[1], x, y, width / 2, height / 2)) {
      ...
      wraith.attr("class", util.getDistance(i[0], i[1], x, y));
...
module.exports = {
	...
	getDistance: function(oX, oY, tX, tY) {
		console.log("dist"+ oX);
	return Math.sqrt((oX-tX)*(oX-tX)+(oY-tY)*(oY-tY));
	},
